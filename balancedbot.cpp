#include "balancedbot.h"
#include "gamemanager.h"

BalancedBot::BalancedBot(const std::string& name, int chips, double tightness, double aggressiveness, int position)
...
PlayerAction BalancedBot::makeDecision(const Gamestate& gameState, GameManager* gameManager) {
...
bool BalancedBot::shouldBeAggressive(const Gamestate& gameState) {
...
PlayerAction BalancedBot::chooseAggressiveAction(const Gamestate& gameState, GameManager* gameManager) {
...
PlayerAction BalancedBot::choosePassiveAction(const Gamestate& gameState, GameManager* gameManager) {
    auto legalActions = gameManager->getLegalActions(gameState.getCurrentPlayerIndex());
    int currentBet = gameState.getCurrentBet();
...
double BalancedBot::getBaseStrength(int handRank) {
    switch(handRank) {
...
double BalancedBot::getCallThreshold() {
...
float BalancedBot::evaluateHandStrength(const Gamestate& gameState) {
    const Hand& playerHand = this->getHand();
    return HandStrengthEvaluator::evaluateHandStrength(playerHand, gameState);
}


int BalancedBot::calculateBetSize(double handStrength, const GameState& gameState) {
...
int BalancedBot::calculateRaiseSize(int currentBet, double handStrength, const GameState& gameState) {
    if (handStrength > 0.8) {
        return currentBet * 3;
    } else if (handStrength > 0.6) {
        return currentBet * 2;
...
double BalancedBot::getPreFlopStrength(const GameState& gameState) {
